...
if(0)
    nrfig = figure(nFigAll);  %movegui( cell2mat(figpos(mod(nfig,length(figpos)-1)+1)) );
...
for(nrs=1:LKanalow)  %2:2) % Petla po szeregach
    nowy=1; tKanal=tic;
    nfig=nfig+1;
    if(0)
...
%         fig=gcf; set(fig,'Units','normalized','Position',[0.11 0.04 0.8 0.82]);
        ...
        Fig=gcf; set(Fig,'Units','normalized','Position',[0.09 0.015 0.8 0.815]);
        %         nrFig = figure(nFig);  %movegui( cell2mat(figpos(mod(nfig,length(figpos)-1)+1)) );
%         Fig=gcf; set(Fig,'Units','normalized','Position',[LKanalow*0.01 -LKanalow*0.01 Xkonc Xkonc]);
    end
    nOs1=1; dno=1; nOsf=Losob;
    for(nosoby=nOs1:dno:nOsf)
        nFigAll=nFigAll0+nosoby; 
        nrfig = figure(nFigAll);  %movegui( cell2mat(figpos(mod(nfig,length(figpos)-1)+1)) );
        %fig=gcf; set(fig,'Units','normalized','Position',[0.039+nosoby*0.011 0.03+nosoby*0.012 0.8 0.82]);  
        fig=gcf; set(fig,'Units','normalized','Position',[0.03+nosoby*0.041 0.02+nosoby*0.02 0.8 0.82]);
        yTrc=zeros(4,Nmax); 
        nYTr1=zeros(1,4); nYTrf=zeros(1,4); nYTrE1=zeros(1,4); nYTrEf=zeros(1,4);
        txtyt=[]; %tic;
        nrS=(nosoby-1)*LKanalow+nrwykr(nrs); nrKanal=nrwykr(nrs);
        nrPliku=1; nrKol=nrs;
        ...
        %for(typS=1:2)
            ...
            for(tS=typS:2:typS2)
                ...
                if(tS==typS)
                    ...
                    if(tS==4) yTrBx=yTrB(typS2,:); yTrB(typS2,:)=sign(yTrBx).*(abs(yTrBx).^(1/Ewykl)); end
                end
            end
         %end
            %typS=wgEn+1;
            if(typS==2 && nrs==1) 
                figure(2); 
                for(ntS=1:4) subplot(2,2,ntS); 
                    plot([1:Nf]*dt,yTrc(ntS,:),'k',[1:Nf-ndel(ntS)]*dt,yTrB(ntS,ndel(ntS)+1:end),'r');
                    switch(ntS) 
                        case 1, txTr='wolny sygn.Y:'; case 2, txTr='wolny sygn.Y^2:';
                        case 3, txTr='szybki sygn.Y:'; case 4, txTr='szybki sygn.Y^2:';
                    end
                    xlabel(sprintf('Trend %s %s_{MTF}=%.0fms %s_B=%.0fms',txTr,txtau,nYTr1(ntS)*dt*1000,txtau,ndel(ntS)*dt*1000));
                    axis('tight'); 
                end;
            end
            typS=wgEn+1; 
            ...
            if(wgEn==0 && nosoby==nOs1)
                ...
                plot([1:Ldsz]*dt,SZEREGI(:,nrS),'k'); axis('tight'); ax=axis; hold on;
                plot([nYTr1(1) nYTr1(1)]*dt,ax(3:4),'r--',[nYTrf(1) nYTrf(1)]*dt,ax(3:4),'r--',[nYTr1(3) nYTr1(3)]*dt,ax(3:4),'g--',[nYTrf(3) nYTrf(3)]*dt,ax(3:4),'g--'), 
                txt1=sprintf('; Okres probk.%st=%.2fms T_{Nyq}=%.2fms; Param. filtrow dol: T_d=%.1fmsek (f_d=%.1fHz) T_g=%.1fmsek (f_g=%.1fHz)',...
                        txDelta,dt*1000,2*dt*1000,Tud*dt*1000,1/(Tud*dt),TuG(1)*dt*1000,1/(TuG(1)*dt));
                xlabel(sprintf('Sygnal zmierzony 1:%d  t[sek]',Ldsz)); %,txDelta,dt));
                hold off;
                subplot(4,4,2); %subplot(4,lcol,ncol);
                plot([1:Ldsz]*dt,Yorsum,'k'); axis('tight'); ax=axis; hold on;
                plot([nYTr1(1) nYTr1(1)]*dt,ax(3:4),'r--',[nYTrf(1) nYTrf(1)]*dt,ax(3:4),'r--',[nYTr1(3) nYTr1(3)]*dt,ax(3:4),'g--',[nYTrf(3) nYTrf(3)]*dt,ax(3:4),'g--'), 
                hold off;
                xlabel(sprintf('%s(1:%d)',txY,Ldsz));
                title(['             ' txg txt1]); %sprintf('; Okres probk.%st=%.4fsek',txDelta,dt)]); 
            end
            if(nosoby==nOs1)
                figure(nfig); 
                subplot(4,lcol,lcol+ncol); 
                %plot([nY1Tr:nYfTr],Yoryg(nY1Tr:nYfTr),'k',[nY1Tr:nYfTr],yTr,'g'), axis('tight'); 
                plot([1:Nf]*dt,Yoryg,'k',[1:Nf]*dt,yTrc(typS,:),'r'), axis('tight'); 
                ax=axis; hold on;
                plot([nYTr1(typS2) nYTr1(typS2)]*dt,ax(3:4),'g--',[nYTrf(typS2) nYTrf(typS2)]*dt,ax(3:4),'g--',[nYTr1(typS) nYTr1(typS)]*dt,ax(3:4),'r--',[nYTrf(typS) nYTrf(typS)]*dt,ax(3:4),'r--');
            ...
            if(nosoby==nOs1) 
                ...
                STrd(nrKanal,nosoby).SrYtr(typS)=SrYtr(typS); STrd(nrKanal,nosoby).SigyTr(typS)=SigyTr(typS);
            end
            ...
            for(nF=1:2) % Rysujemy na nFig=11 i na nfig
                ...
                plot([1:Nf]*dt,sygnTr,kol,[1:Nf-ndel(typS)]*dt,sygnTrB(ndel(typS)+1:Nf),[kol '--']);
                   if( Nf == 2) hold on; plot( [1:Nf]*dt,sygnTrB,[kol ':']); end; 
                axis('tight'); hold on; ax=axis;
                if(nowy) 
                    plot([nYTr1(typS2) nYTr1(typS2)]*dt,ax(3:4),'g--',[nYTrf(typS2) nYTrf(typS2)]*dt,ax(3:4),'g--',[nYTr1(typS) nYTr1(typS)]*dt,ax(3:4),'r--',[nYTrf(typS) nYTrf(typS)]*dt,ax(3:4),'r--');
                    xlabel(sprintf('Kanal %d: yTr(%d:%d)',nrwykr(nrs),nYTr1(typS),nYTrf(typS))); 
                    if(nF==1 && nrs==2) title(['        ' tx '  ' txg sprintf('; %st=%.4fmsek T_{Nyq}=%.4fmsek',txDelta,dt*1000,2000*dt)]); end
                    txl=sprintf('Kan.%d: yTr_{MTF}(%c) yTr_B(%c--)',nrwykr(nrs),kol,kol); 
                    if(nF==2) txl=[txl sprintf(' %s_B=%.1fms',txtau,ndel(typS)*dt*1000)]; end
                    xlabel(txl); 
                end
                hold off;
                if(nF==1) 
                    % ========= Zapisujemy szeregi w strukturze STrd =====
                    STrd(nrKanal,nosoby).yTrc(typS,:)=sygnTr; STrd(nrKanal,nosoby).yTrB(typS,:)=sygnTrB; 
                    STrd(nrKanal,nosoby).ndel(typS)=ndel(typS); 
                    STrd(nrKanal,nosoby).nYTr1(typS)=nYTr1(typS); STrd(nrKanal,nosoby).nYTrf(typS)=nYTrf(typS);
                    % ====================================================
            ...
            if(nosoby==nOs1)
                ...
                plot([1:Nf]*dt,yxf,kol); %plot([nY1-npC+1:nYf+N2],yxf,kol);
                axis('tight'); ax=axis; axis([[1 Nf]*dt ax(3:4)]); hold on; 
                plot([nYTr1(typS2) nYTr1(typS2)]*dt,ax(3:4),'g--',[nYTrf(typS2) nYTrf(typS2)]*dt,ax(3:4),'g--',[nYTr1(typS) nYTr1(typS)]*dt,ax(3:4),'r--',[nYTrf(typS) nYTrf(typS)]*dt,ax(3:4),'r--'); 
                %hold off;
                xlabel(sprintf('Trend szybki yTrc/B(%d:%d); %s_B=%.0fms',nYTr1(typS2)-Nd,nYTrf(typS2)+Nd,txtau,(ndel(typS2))*dt*1000));
                figure(nfig);
                subplot(4,lcol,lcol+ncol); kol=Kolor(nosoby);
                hold on; plot([1:Nf]*dt,yTrBx,'g'); %plot([nY1Tr:nYfTr],yTrB(nY1Tr:nYfTr),'g');
                hold off;
            end
            ...
            if(nosoby==nOs1)
                ...
                for(m=1:2:3)
                    lAa=iA01(wgEn+m); if(lAa>length(x))  x=wTuf*[0:lAa-1]; end, %xo=wTuf*wo*[0:lAa-1]; 
                ...
                hold off;
                txx=sprintf('Filtry dol: k,b-MTF, r,m-Butt^%d; A(f/f_d) f_d=%.1fHz',rzadB,1/(Tud*dt));
                xlabel(txx); ylabel('Amplituda'); %title(tx);
                x=wTuf*[0:lAx-1]; 
                if(typS==1)
                    subplot(4,4,4); %subplot(4,lcol,ncol); %subplot(2,2,2); %hold on;
                    lAfx=min(lAf,length(Ampd(1,:)));
                    plot(x(1:lAfx),Ampd(typS,1:lAfx),'r',x(1:lAfx),Ampd(typS2,1:lAfx),'b',x(1:lAfx),Afc(1:lAfx),'k'); hold off; %,x,Afc1(1:lAf),'g');
                    ...
                    txx=sprintf('A(f/f_d); f_d=%.1fHz f_g=%.1fHz',1/(Tud*dt),1/(TuG(1)*dt));
                    if(LFg==2) txx=[txx sprintf(' f_{g2}=%.1fHz',1/(TuG(2)*dt))]; end
                    xlabel(txx); ylabel('Amplituda'); %title(tx);
                end
            end
            ...
            if(nosoby==nOs1)
                ...
                xlabel(txx); ylabel('Amplituda'); xlabel('Widmo amplit.sklad.srednioczest. Ayf(f/f_d)');
                hold off;
                ...
                ylabel('Amplituda'); xlabel('Widmo amplit.trendu ATr(f/f_d)');
            end
            %fprintf(1,'\nSzereg %d. Czas obliczeñ: %.2f ',nrs,toc); 
        end  % wersja Y^2 Y
        nowy=0;
        figure(nFigAll);
        nY1=STrd(nrKanal,nosoby).nYTr1(1); nYf=STrd(nrKanal,nosoby).nYTrf(1);
        nYE1=STrd(nrKanal,nosoby).nYTr1(2); nYEf=STrd(nrKanal,nosoby).nYTrf(2);
        subplot(4,4,nrs); del=STrd(nrKanal,nosoby).ndel;
        plot([1:Nf]*dt,STrd(nrKanal,nosoby).yTrc(1,:),'b',[1:Nf]*dt,STrd(nrKanal,nosoby).yTrc(2,:),'k',...
            [1:Nf-del(1)]*dt,STrd(nrKanal,nosoby).yTrB(1,del(1)+1:end),'m',...
            [1:Nf-del(2)]*dt,STrd(nrKanal,nosoby).yTrB(2,del(2)+1:end),'r'); axis('tight');
        ax=axis; hold on;
        plot([nY1 nY1]*dt,ax(3:4),'g--',[nYE1 nYE1]*dt,ax(3:4),'r--',[nYf nYf]*dt,ax(3:4),'g--',[nYEf nYEf]*dt,ax(3:4),'r--');
        hold off;
        xlabel(sprintf('Kan.%d %s_{MTF}=[%.0f %.0f]ms; %s_B=[%.0f %.0f]ms',...
            nrKanal,txtau,(nY1-1)*dt*1000,(nYE1-1)*dt*1000,txtau,del(1)*dt*1000,del(2)*dt*1000));
        subplot(4,4,2); txt4=sprintf('Estymaty trendu wolnego dla osoby %d:  Okres probk.%st=%.2fms T_{Nyq}=%.2fms; Param. filtrow: T_d=%.1fms (f_d=%.1fHz) T_g=%.1fms (f_g=%.1fHz). yTrc(Y)_b, yTrc(Y^%d)_k, yTrB(Y)_m, yTrB(Y^%d)_r',...
            nosoby,txDelta,dt*1000,2*dt*1000,Tud*dt*1000,1/(Tud*dt),TuG(1)*dt*1000,1/(TuG(1)*dt),Ewykl,Ewykl);
        title(['                                   ' txt4]);
        subplot(4,4,8+nrs);
        dTr(Nf)=0;  dTr(nY1:nYf)=STrd(nrKanal,nosoby).yTrc(2,nY1:nYf)-STrd(nrKanal,nosoby).yTrc(1,nY1:nYf);
        dTrB(Nf)=0; dTrB(nYE1:nYEf)=STrd(nrKanal,nosoby).yTrc(2,nYE1:nYEf)-STrd(nrKanal,nosoby).yTrB(2,[nYE1:nYEf]+del(2));
        plot([1:Nf]*dt,dTr,'k',[1:Nf]*dt,dTrB,'r'); axis('tight');% dTr/STrd(1,1).SigyTr(2))
        xlabel(sprintf('Kan.%d: [yTr2-yTr1]_k i [yTr2-yTrB2(+%s_B)]_r',nrKanal,txtau));
        ax=axis; hold on;
        plot([nY1 nY1]*dt,ax(3:4),'g--',[nYE1 nYE1]*dt,ax(3:4),'r--',[nYf nYf]*dt,ax(3:4),'g--',[nYEf nYEf]*dt,ax(3:4),'r--');
        hold off;
...
% Przyk³ad syntezy z wykresami:fu
