...
tic;
 for(nrf=1:LFd)
    ...
    nx=find(TypyZ==ntypZ); if(length(nx)==0) ntypZ=TypyZ(1); end
        fname=sprintf('MTFd%d_%d_%d.mat',nrf,ntypZ,round(Tu));
        fnamec=sprintf('MTFd%d_%d_%d.csv',nrf,ntypZ,round(Tu));
    fp=fopen(fname,'r');
    if(fp>1) fclose(fp);
        load(fname);
        %W=[M lf N1 N2]; for(i=1:N1) W=[W F0(:,i)']; end; W=[W Fzw']; for(i=1:N2) W=[W Ff(:,i)']; end, 
        dane=csvread(fname); Ff=[]; Fzw=[]; F0=[];
        M=dane(1); lf=dane(2); N1=dane(3); N2=dane(4); ip=4;
        ip=ip+1; maxFf=dane(ip); i0=ip+1; ik=lf+ip; for(i=1:N1) Ff(1:lf,i)=dane(i0:ik); i0=i0+lf; ik=ik+lf; end;  Ff=Ff*maxFf; ip=ik-lf;
    ...
    MTFd(ntypZ,nrf).Ff=Ff; 
    if(fp<2)
        W=[M lf N1 N2]; maxFf=max(max(abs(Ff))); W=[W maxFf]; for(i=1:N1) W=[W (Ff(:,i)')/maxFf]; end;
        maxFzw=max(abs(Fzw)); W=[W maxFzw]; W=[W (Fzw')/maxFzw]; 
        %maxFf=max(max(abs(Ff))); W=[W maxFf]; for(i=1:N2) W=[W (Ff(:,i)')/maxFf]; end, 
        csvwrite(fnamec,W); 
    end
    fp=fopen(fname,'r');
    if(fp>1) fclose(fp);
        save(fname,'M', 'F0', 'Fzw', 'Ff', 'Ffshf','ktory')
    end
    if(nrf==1)
        Fzwd=Fzw; MTud=M; Ffd=Ff; lfd=lf; N2d=length(Ffd(1,:)); Lzwd=lfd;
...
jestButter=0; lw=length(Om); nfpocz=5;
if(lT0==0)
...
MTF=Fzwc;
for(nf=2:2*LFd)
    A=abs(fft([MTF' zeros(1,lT0-length(MTF))])); A=A(1:Lxf);
    nx=find(A<1/sqrt(2)); if(~isempty(nx)) nhP(nf)=nx(1); nx=[]; else nhP(nf)=Lxf; end
    nx=find(A(nhP(nf):Lxf)<=0.01); if(~isempty(nx)) nA01(nf)=nx(1); iA01=nx(1); nx=[]; else nA01(nf)=Lxf; end
    Amp(nf,:)=A;
