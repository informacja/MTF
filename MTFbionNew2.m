...
Lk = 0; Lgestow=round(LSzeregow/2); Losob=2; %Losob = size(SZEREGI,2)/8; Lgestow = LSzeregow/8;  %LC = LSzeregow;Lgestow = round(LC / 2); Losob = 2;
    ...
    for (nosoby = nOs1:dno:nOsf)
        ...
        for (wgEn = 0:LwgE)
            ...
            for (nfw = 1:Lfw)
                ...
                Tu = 6; %Tud/20; % MTud*lfA; % PrzedziaĹ‚ wygĹ‚adzania widma trendu
            ...
            if (nrs == 1)
                ...
                for (i = 1:lT) if(Afd(i) >= 1 / sqrt(2)); ihP = i; else break; end; end, wPol = 2 * pi * ihP / lT;
                [bfB, afB, AmpB, Phase, ihPB, iA02, tauhP, tauA02, tauTu] = designButter(Tud, 5, lT);
                % Przykład syntezy z wykresami:
                %% designButter(Tud,[3 4 5],lT,1,'cgkrbm','Charakt.Bodego dla filtrow Butter(3 4 5)_{cgk} i MTF: Fzwd_r Fzwc_b Ffd(:,N2d)_m Ffc_{c--}',Fzwd,Fzwc, Ffd(:,length(Ffd(1,:))), Ffc);
                %  designButter(Tud,[3 4 5],lT,1,'cgkrbm','Charakt.Bodego dla filtrow Butter(3 4 5)_{cgk} i MTF: Fzwd_r Fzwc_b ',Fzwd,Fzwc); %
            end
            ...
            % ............... Teraz wykresy dla yfB .....................
            ndel = -round(tauhP); txtau = '\tau';
            figure(nFig); subplot(4, lcol, nCol); kol = Kolor(nosoby);
            sygnTr = (yfB - mean(yfB)) / SigyTr(wgEn + 1) + SrYtr(wgEn + 1);
            ...
            if (nosoby == nOs1)
                ...
                hold on; plot([1:Nf], yfB, 'g'); %plot([nY1Tr:nYfTr],yfB(nY1Tr:nYfTr),'g');
                hold off;
                % ============================================
            ...
            if (wgEn == 0 && nosoby == nOs1)
                ...
                plot(xn * 2048 / (K * N - 1), Asn(xn + 1), 'r', x1 * 2048 / (K * N - 1), As(x1 + 1), 'k'); xlabel(sprintf('Spectrum: zmierzony_k, zsumowany_r')); axis('tight');
